...
##### [Version 0.5.4](https://github.com/lazarusllong/sqldeveloper-package/releases/tag/0.5.4):
- **changelog:**
  - Updated wrapper script copyright to include 2018
- **downloads:**
  - [**`sqldeveloper-package_0.5.4_all.deb`**](https://github.com/lazarusllong/sqldeveloper-package/releases/download/0.5.4/sqldeveloper-package_0.5.4_all.deb) (_binary package_)
  - [`sqldeveloper-package_0.5.4.dsc`](https://github.com/lazarusllong/sqldeveloper-package/releases/download/0.5.4/sqldeveloper-package_0.5.4.dsc) (_source description file_)
  - [`sqldeveloper-package_0.5.4.tar.xz`](https://github.com/lazarusllong/sqldeveloper-package/releases/download/0.5.4/sqldeveloper-package_0.5.4.tar.xz) (_source code archive_)
  - [`v0.5.3_v0.5.4.debdiff`](https://github.com/lazarusllong/sqldeveloper-package/releases/download/0.5.4/v0.5.3_v0.5.4.debdiff) (_Debian diff from previous version_)

##### [Version 0.5.3](https://github.com/lazarusllong/sqldeveloper-package/releases/tag/0.5.3):
- **changelog:**
  - Fixed `LD_LIBRARY_PATH` initialization, which was rendering the wrapper script unusable, thus breaking the package
  - Fixed _QA_ "**unsatisfiable Depends**" alerts by disabling cross-building
...
- **changelog:**
  ...
  - Multiple versions can now coexist, `sqldeveloper.[`_`upstream version`_`]` will invoke a specific version of _Oracle SQL Developer_ while `sqldeveloper` takes advantage of _Debian_'s `alternatives` system and, when left in auto mode, will always invoke the highest version installed
  - Exposed _Oracle CLI for SQL Developer_, a non-gui interface for scripting or headless control of _Oracle SQL Developer_. Following the multiple versions  coexistence concept, `sdcli.[`_`upstream version`_`]` and `sdcli` will behave as described above
  - Since _SQL Developer_ [_v4.2_](http://www.oracle.com/technetwork/developer-tools/sql-developer/downloads/sqldev-downloads-42-3802334.html), _Oracle_ started to bundle _Oracle SQL Developer Command-Line_ (**_SQLcl_**) with _Oracle SQL Developer_. When available, and as an alternative to the standalone package (see [`sqlcl-package`](https://lazarusllong.github.io/sqlcl-package)), `make-sqldeveloper-package` will expose it. Since there is a name conflict with content of package [_`parallel`_](https://tracker.debian.org/pkg/parallel), the upstream binary `sql` will be renamed, and following the above multiple versions coexistence concept, `sql.[`_`upstream version`_`].bundled` will invoke a specific _SQLcl_ version while `sqlcl.bundled` takes advantage of _Debian_'s `alternatives` system and, when left in auto mode, will always invoke the highest _bundled_ version installed, and `sqlcl` will invoke the highest version installed (either _standalone_ or _bundled_, in this order)
